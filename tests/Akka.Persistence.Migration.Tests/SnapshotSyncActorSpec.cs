...
public class SnapshotSyncActorLastSnapshotsSpec : Akka.Hosting.TestKit.TestKit
{
    private readonly MsSqliteContainer _targetFixture;
    private TestProbe? _probe;
    private IActorRef? _syncActor;
    private ITestSnapshotStore? _snapshots;
    private MigrationOptions? _options;

    public SnapshotSyncActorLastSnapshotsSpec(ITestOutputHelper output)
        : base(nameof(SnapshotSyncActorLastSnapshotsSpec), output)
    {
    ...
    [Fact(DisplayName = "SnapshotSyncActor should load last snapshot")]
    public async Task SnapshotSyncReadTest()
    {
        await Initialize(true);
        
        _syncActor = Sys.ActorOf(Props.Create(() => new SnapshotSyncActor(_options!, _probe!, "a")), "snapshot-sync-actor");
    ...
    {
        await SnapshotSyncReadTest();

        await WatchAsync(_syncActor);
        _syncActor.Tell(PoisonPill.Instance);
        await ExpectTerminatedAsync(_syncActor);
        ...
        _syncActor.Tell(SnapshotSyncProtocol.GetCount.Instance, TestActor);
        var count = await ExpectMsgAsync<SnapshotSyncProtocol.SnapshotSyncCount>(10.Minutes());
        count.Count.Should().Be(0);
